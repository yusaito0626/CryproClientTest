...
Thread.Sleep(5000);
string ordId = await cl.placeNewSpotOrder(Exchange.Bybit, "ETH", "USDT", CryptoExchange.Net.SharedApis.SharedOrderSide.Buy, CryptoExchange.Net.SharedApis.SharedOrderType.Limit, (decimal)0.001, 4000);
Console.WriteLine("Order ID:" + ordId);
Thread.Sleep(5000);
Console.WriteLine("Now cancelling an order to Bybit");
string cancelling = await cl.placeCancelSpotOrder(Exchange.Bybit, "ETH", "USDT", ordId);
Console.WriteLine("Done Canceled ID:" + cancelling);
